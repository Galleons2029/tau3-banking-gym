...
This script loads a results.json file from a tau3 run() and reviews
...
Usage:
    ...
    python -m tau3.scripts.review_conversation results.json

    # User simulator only review
    python -m tau3.scripts.review_conversation results.json --mode user

    # With custom output path
    python -m tau3.scripts.review_conversation results.json -o review.json

    # With a custom LiteLLM-compatible review model
    python -m tau3.scripts.review_conversation results.json --review-model gpt-4.1
"""
...
from tau3.config import DEFAULT_LLM_EVAL_USER_SIMULATOR
...
from tau3.evaluator.auth_classifier import display_auth_classification
from tau3.evaluator.review_llm_judge_user_only import UserOnlyReviewer
from tau3.evaluator.reviewer import ReviewMode, review_simulation
...
def review(
    ...
    if log_llm:
        from tau3.utils.llm_utils import set_llm_log_dir
...
Examples:
  ...
  python -m tau3.scripts.review_conversation run results.json

  # Review all results.json files in a directory
  python -m tau3.scripts.review_conversation run data/simulations/

  # User simulator only review
  python -m tau3.scripts.review_conversation run results.json --mode user

  # With detailed output for each simulation
  python -m tau3.scripts.review_conversation run results.json --show-details

  # Use a different LiteLLM-compatible review model
  python -m tau3.scripts.review_conversation run results.json --review-model gpt-4.1
        """,
    )
...
Examples:
  ...
  python -m tau3.scripts.review_conversation display review.json

  # Show only simulations with errors
  python -m tau3.scripts.review_conversation display review.json --errors-only

  # Show a specific simulation
  python -m tau3.scripts.review_conversation display review.json --simulation abc123

  # Non-interactive: show all
  python -m tau3.scripts.review_conversation display review.json --all
        """,
    )
...
def find_results_files(path: Path) -> list[Path]:
    ...
    results_files = []
    # Look for results.json in immediate subdirectories (typical tau3 structure)
