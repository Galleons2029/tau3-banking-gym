...
def main():
    ...
    servers = [
        ("sh scripts/start_tau3_server.sh", 8001),
    ]

    # Set up signal handlers
    def signal_handler(signum, frame):
        logger.info("\nReceived shutdown signal, cleaning up...")
        # Only clean up the tau3 server port since React script handles its own cleanup
    ...
    logger.info("Starting Tau3 server...")
    try:
        ...
        # Cleanup on exit - only clean up tau3 server
