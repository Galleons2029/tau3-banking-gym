"""tau3 worker process: leases simulation units from a controller, runs them,
...
Run as ``tau3 worker --controller http://host:port --slots 10`` or
``python -m tau3.runner.worker ...``. See docs/designs/parallel-runner.md.
"""
...
class HeartbeatThread(threading.Thread):
    ...
    def __init__(
        ...
        super().__init__(daemon=True, name="tau3-worker-heartbeat")
        self._client = client
...
def execute_lease(payload: dict) -> SimulationRun:
    ...
    from tau3.runner.batch import _BatchContext, run_unit
    from tau3.runner.helpers import get_info
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="tau3 worker: executes simulations for a tau3 controller."
    )
    parser.add_argument(
        "--controller",
        required=True,
