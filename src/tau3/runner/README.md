...
```python
from tau3.runner import build_text_orchestrator, run_simulation
...
## Relationship to `tau3.run`

The `tau3.run` module is the original monolithic implementation. The `tau3.runner` package is its modular replacement. Both coexist:

- **`tau3.run`**: Still works for all existing callers (CLI, tests, scripts). Not deprecated yet.
- **`tau3.runner`**: The recommended API for new code. Cleaner separation of concerns.

Over time, `tau3.run` will be thinned out to re-export from `tau3.runner`.
