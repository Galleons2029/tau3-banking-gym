...
(``tau3 worker`` / ``python -m tau3.runner.worker``) lease units over HTTP,
...
Locally, ``tau3 run --workers N`` binds 127.0.0.1 on an ephemeral port and
spawns its own workers. Remote workers are the same binary pointed at a
routable address, authenticated with TAU3_CONTROLLER_TOKEN.
"""
...
from tau3.runner.batch import BatchPrep
from tau3.runner.progress import StatusMonitor
...
from tau3.utils.llm_utils import llm_log_mode
from tau3.utils.utils import DATA_DIR, get_now

AUTH_TOKEN_ENV = "TAU3_CONTROLLER_TOKEN"
...
def _rekey(unit: WorkUnit, run_id: str) -> str:
    from tau3.runner.work import make_unit_id
...
class Controller:
    ...
    def build_app(self) -> FastAPI:
        app = FastAPI(title="tau3 controller")
