...
This folder contains the domains for the Tau3 project.
...
- `data_model.py`: Defines the data models for the domain. 
    - Implements a subclass of `DB` (e.g. `FlightDB(DB)`) which is the base class for all domain databases (defined in `tau3/environment/db.py`).
...
- `tools.py`: Defines the tools for the domain. 
    - Implements a subclass of `ToolKitBase` (e.g. `AirlineTools(ToolKitBase)`) which is the base class for all domain toolkits (defined in `tau3/environment/toolkit.py`). 
...
All the data for the domain is stored in `data/tau3/domains/<domain_name>` folder.
Should contain:
...
[`src/tau3/data_model/tasks.py`](../data_model/tasks.py)) with an
...
- **Retrieval pipeline**: Uses the `src/tau3/knowledge/` module for embeddings, BM25, grep, reranking, and sandboxed shell access. See `src/tau3/knowledge/README.md` for config details.

## Registering your domain
To make it easy for people to use your domain, you need to register your `get_environment`, `get_tasks`, and `get_tasks_split` functions in Tau3 `registry.py` file.

In `registry.py`:
```python
from tau3.domains.your_domain.environment import get_environment as your_domain_get_environment
from tau3.domains.your_domain.environment import get_tasks as your_domain_get_tasks
from tau3.domains.your_domain.environment import get_tasks_split as your_domain_get_tasks_split
...
