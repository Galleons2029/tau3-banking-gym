...
Every domain in `src/tau3/domains/<name>/` MUST have:
...
- `utils.py` — data paths pointing to `data/tau3/domains/<name>/`
- `__init__.py`
...
After creating or renaming a domain, register it in `src/tau3/registry.py`:
...
Domain data lives in `data/tau3/domains/<name>/`. Be careful modifying `db.json`/`db.toml` and `tasks.json` — the framework depends on these at runtime. Always run `tau3 check-data` after changing data files.
...
- **Extended data directory**: `data/tau3/domains/banking_knowledge/` contains `documents/` (700+ JSON docs), `prompts/` (per-variant policy templates), and `tasks/` (individual task JSON files) in addition to `db.json`.
- **Retrieval pipeline**: Uses the `src/tau3/knowledge/` module for embeddings, BM25, grep, reranking, and sandboxed shell access. See `src/tau3/knowledge/README.md`.
