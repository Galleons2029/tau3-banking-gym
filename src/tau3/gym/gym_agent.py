...
from tau3.agent.llm_agent import LLMAgent
...
from tau3.user.user_simulator import DummyUser, UserSimulator
...
from tau3.utils.tools import parse_action_string, to_functional_format
...
def register_gym_agent() -> None:
    ...
    register(
        id=TAU_BENCH_ENV_ID,
        entry_point="tau3.gym.gym_agent:AgentGymEnv",
    )
    register(
        id=TAU_BENCH_USER_ENV_ID,
        entry_point="tau3.gym.gym_agent:UserGymEnv",
    )
...
class AgentGymEnv(gym.Env):
    """
    A Gymnasium environment that wraps the Tau3 simulation system.

    This environment provides a standard gym interface for interacting with
    Tau3 simulations. It manages the lifecycle of orchestrators, agents,
    ...
    - The internal Tau3 orchestrator running in a separate thread
    ...
    def step(self, action: str) -> tuple[str, float, bool, bool, dict]:
        ...
        and the internal Tau3 simulation running in a separate thread.
    ...
    def _get_reward(self) -> tuple[float, str]:
        ...
        This method evaluates the simulation using the Tau3 evaluation
