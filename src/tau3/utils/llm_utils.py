...
def to_tau3_messages(
    messages: list[dict], ignore_roles: set[str] = set()
) -> list[Message]:
    """
    Convert a list of messages from a dictionary to a list of Tau3 messages.
    """
    tau3_messages = []
    for message in messages:
        ...
        if role in ignore_roles:
            continue
        if role == "user":
            tau3_messages.append(UserMessage(**message))
        elif role == "assistant":
            tau3_messages.append(AssistantMessage(**message))
        elif role == "tool":
            tau3_messages.append(ToolMessage(**message))
        elif role == "system":
            tau3_messages.append(SystemMessage(**message))
        else:
            raise ValueError(f"Unknown message type: {role}")
    return tau3_messages


def to_litellm_messages(messages: list[Message]) -> list[dict]:
    """
    Convert a list of Tau3 messages to a list of litellm messages.
    """
    litellm_messages = []
    for message in messages:
