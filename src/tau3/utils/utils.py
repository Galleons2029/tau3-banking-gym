...
# TAU2_DATA_DIR is the pre-rename name, still honoured so that non-editable
# installs configured before the tau2 -> tau3 rename keep working.
DATA_DIR_ENV = os.getenv("TAU3_DATA_DIR")
if not DATA_DIR_ENV:
    DATA_DIR_ENV = os.getenv("TAU2_DATA_DIR")
    if DATA_DIR_ENV:
        logger.warning(
            "TAU2_DATA_DIR is deprecated and will be removed; rename it to TAU3_DATA_DIR"
        )
...
if not DATA_DIR.exists():
    ...
    logger.warning(
        "Set TAU3_DATA_DIR environment variable to point to your data directory"
    )
...
def get_tau2_version() -> str:
    """Get the installed tau3 package version.

    Keeps its pre-rename name because it feeds the leaderboard submission
    schema's frozen ``tau2_bench_version`` field.
    """
    try:
        return importlib.metadata.version("tau3")
